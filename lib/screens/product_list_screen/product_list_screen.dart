...
class ProductListScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
     // เรียกใช้งานฟังก์ชันเพื่อโหลดสินค้าล่าสุด เมื่อหน้าเปิดครั้งแรก
    controller.fetchLatestProducts(isInitialLoad: true);
    
    scrollController.addListener(() {
     //(scrollController.position.pixels); // ตำแหน่งการเลื่อนปัจจุบันในแนวตั้ง 
    //(scrollController.position.maxScrollExtent); //ระยะทางสูงสุดที่สามารถเลื่อนได้
      if (scrollController.position.pixels >=
              scrollController.position.maxScrollExtent && // ตรวจสอบว่าเลื่อนถึงท้ายสุด
          !controller.isLoadingLatestProducts.value && // ตรวจสอบว่าขณะนี้ไม่ได้โหลดข้อมูล
          controller.nextCursor != null //ตรวจสอบว่ามีข้อมูลเพิ่มเติมหรือไม่ (pagination) 
          && controller.latestProducts.length < 100) { 
        // ถ้าผ่านเงื่อนไข จะเรียกฟังก์ชันเพื่อโหลดสินค้าล่าสุเพิ่มเติม
